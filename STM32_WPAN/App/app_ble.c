...
uint8_t a_AdvData[25] =
{
  ...
  7, AD_TYPE_COMPLETE_LOCAL_NAME, 'W', 'B', '_', 'O', 'O', 'B',  /* Complete name */
  13, AD_TYPE_MANUFACTURER_SPECIFIC_DATA, 0x01, 0x83, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0xDE /*  */, 0xAD /*  */, 0xBE /*  */, 0xEF /*  */, 0x12 /*  */, 0x34 /*  */,
};
        ...
        case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE:
          ...
          if (p_pairing_complete->Status != 0)
          {
            uint8_t *p_reasons[] = { "OK",
              ...
              "OOB NOT AVAILABLE",
              "AUTH REQ CANNOT BE MET",
              "CONFIRM VALUE FAILED",
              "PAIRING NOT SUPPORTED",
              "INSUFF ENCRYPTION KEY SIZE",
              "CMD_NOT_SUPPORTED",
              "UNSPECIFIED_REASON",
              "VERY_EARLY_NEXT_ATTEMPT",
              "SM_INVALID_PARAMS",
              "SMP_SC_DHKEY_CHECK_FAILED",
            ...
            // [STM] If OOB pairing fails, disconnect with reason 0x3B (Unacceptable connection parameters)
            hci_disconnect(BleApplicationContext.BleApplicationContext_legacy.connectionHandle, 0x3B);
            // [STM] Expect to see reason 0x16 (LOCAL HOST TERMINATED CONNECTION) eventually
          }
...
void APP_BLE_Key_Button3_Action(void)
{
  ...
  APP_DBG_MSG("APP_BLE_Key_Button3_Action\n");
...
void APP_BLE_set_oob(app_ble_oob_data_t *p_data)
{
  ...
  //p_data->ra[1]++; // [STM] Test code for tampering OOB ra data, enable this code to SABOTAGE OOB pairing
  status = aci_gap_set_oob_data(0x01, // [STM] 0x01: Remote device (Central)
                                p_data->address_type, p_data->address,
                                0x01, // [STM] 0x01: Data type Random 
                                p_data->ra_size, p_data->ra);
  
  APP_DBG_MSG("==>> aci_gap_set_oob_data (ra Random data) result = 0x%X\r\n", status);
  ble_oob_data_log(p_data->ra_size, p_data->ra);
  
  //p_data->ca[1]++;  // [STM] Test code for tampering OOB Ca data, enable this code to SABOTAGE OOB pairing
  status = aci_gap_set_oob_data(0x01, // [STM] 0x01: Remote device
                                p_data->address_type, p_data->address,
                                0x02, // [STM] 0x02: Data type Confirm 
                                p_data->ca_size, p_data->ca);
  
  APP_DBG_MSG("==>> aci_gap_set_oob_data (Ca Confirm data) result = 0x%X\r\n", status);
  ble_oob_data_log(p_data->ca_size, p_data->ca);
}
...
static void ble_oob_data_log (uint32_t size, uint8_t *p_data)
{
  ...
  sprintf((char *) msg, "Data (size = %d) =", size);
